...
exports.marshallValue = exports.marshallItem = void 0;
const InvalidValueError_1 = require("./InvalidValueError");
const InvalidSchemaError_1 = require("./InvalidSchemaError");
const dynamodb_auto_marshaller_1 = require("@awslabs-community-fork/dynamodb-auto-marshaller");
...
function marshallValue(schemaType, input) {
    ...
    if (schemaType.type === 'Any') {
        ...
        const marshaller = new dynamodb_auto_marshaller_1.Marshaller({ onEmpty, onInvalid, unwrapNumbers });
    ...
    if (schemaType.type === 'Set') {
        if (schemaType.memberType === 'Binary') {
            if (!(input instanceof dynamodb_auto_marshaller_1.BinarySet)) {
                const set = new dynamodb_auto_marshaller_1.BinarySet();
                for (const item of input) {
                    set.add(marshallBinary(item));
                }
                input = set;
            }
            return marshallSet(input, marshallBinary, (bin) => bin.byteLength === 0, 'BS');
        }
        ...
        if (schemaType.memberType === 'String') {
            ...
            return marshallSet(input, marshallString, (string) => string.length === 0, 'SS');
        }
    ...
    if (schemaType.type === 'Tuple') {
        return {
            L: schemaType.members
                .map((type, index) => marshallValue(type, input[index]))
