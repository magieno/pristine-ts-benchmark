...
exports.SnsEventMapper = void 0;
const core_1 = require("@pristine-ts/core");
const common_1 = require("@pristine-ts/common");
const tsyringe_1 = require("tsyringe");
const sns_event_payload_1 = require("../event-payloads/sns.event-payload");
const sns_event_type_enum_1 = require("../enums/sns-event-type.enum");
const sns_model_1 = require("../models/sns.model");
const sns_message_attribute_model_1 = require("../models/sns-message-attribute.model");
...
let SnsEventMapper = class SnsEventMapper {
     ...
     */
    findEnum(eventName) {
        const keys = Object.keys(sns_event_type_enum_1.SnsEventType).filter(key => isNaN(Number(key)));
        for (const key of keys) {
            if (sns_event_type_enum_1.SnsEventType[key] === eventName) {
                return sns_event_type_enum_1.SnsEventType[key];
            }
        }
        return sns_event_type_enum_1.SnsEventType.UnknownSnsEvent;
    }
    /**
    ...
    map(rawEvent, executionContext) {
        ...
        for (const record of rawEvent.Records) {
            const event = new core_1.Event(this.findEnum(record.Sns.Type), new sns_event_payload_1.SnsEventPayload());
            ...
            event.payload.sns = new sns_model_1.SnsModel();
            ...
            if (record.Sns.hasOwnProperty("MessageAttributes")) {
                ...
                for (const key in record.Sns.MessageAttributes) {
                    if (record.Sns.MessageAttributes.hasOwnProperty(key)) {
                        const attribute = new sns_message_attribute_model_1.SnsMessageAttributeModel();
                        attribute.key = key;
...
exports.SnsEventMapper = SnsEventMapper;
